class PhysicsFactory {
    constructor() {
        ...
        this.world = new Goblin.World(this.broadphase, new Goblin.NarrowPhase(), new Goblin.IterativeSolver());
        this.body_to_mesh_map = {};
        this.endPoint = new Goblin.Vector3();
        this.invertNormalGoblin = new Goblin.Vector3();
    ...
    raycast(startPoint, direction) {
        // todo
        // dont intersect with Ghost Bodys!
        // dont intersect with player cylinder

        var distance = 40;
        this.endPoint.addVectors(startPoint, direction.multiplyScalar(distance));
        // console.log( startPoint, endPoint );

        var intersections = this.getWorld().rayIntersect(startPoint, this.endPoint);

        return intersections;
    };

    calculateHitForce(distance, power) {
    ...
    applyDirectionalImpulse(target, fromVector, power) {
        // console.log( target );
        ...
        // no obvious errors
        this.invertNormalGoblin.subtractVectors(point, fromVector);

        var distance = this.invertNormalGoblin.length();
        var impactForce = this.calculateHitForce(distance, power);

        this.invertNormalGoblin.normalize();
        this.invertNormalGoblin.scale(impactForce);

        body.applyForceAtWorldPoint(this.invertNormalGoblin, point);
    };

    ghostBody(dimension, position, rotation) {
    ...
    makeStaticBox(dimension, position, rotation, mergeGeometry, materialIndex, shape, transform) {
        var goblinDimension = dimension.clone().divideScalar(2);
    ...
    getProxyMesh(object, geometryType) {
    ...
    createPlane(orientation, half_width, half_length, mass, material, name) {
        var thickness = 0.4;
